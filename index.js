...
app.get("/", (request, response) => {
  ...
  http.get(`http://satisfying-sugary-bumper.glitch.me`);
...
setInterval(() => {
  http.get(`http://satisfying-sugary-bumper.glitch.me`);
  http.get(`http://tester-tcv2.glitch.me`);
...
bot.on("message", async message => {
    ...
    else
    {
        ...
        if(message.content.startsWith(prefix) || message.content.startsWith(prefix.toUpperCase()))
        {
            // if(message.author.id != '292675388180791297')
            // {
            //   return message.channel.send('**Server Update!!** \n*Bot will be unavailable for couple of Hours*').then(msg => msg.delete(5000));
            //   console.log(`Request by ${message.author.username} from ${message.guild.name} server`);
            // }else{
            //   message.channel.send(`Server Update in Process`).then(msg => msg.delete(1000));
            // }
        
          
            IndexUserID = message.author.id
            UserNameDS = message.author.username
            IndexGuildID = message.guild.id
            GuildNameDS = message.guild.name
            ...
            {
                if(!serveruser) await startServer(bot,message,args,DBserver,serveruser)
                //await console.log(`+ + + User: ${UserNameDS} | ${IndexUserID} + + +\n+ + + Guild: ${GuildNameDS} | ${IndexGuildID} + + +`)
                await CommandHandler()
        ...
        function CommandHandler(){
            ...
            if(commandfile_3) {   
                  commandfile_3.run(bot,message,args,DBprofile,DBstats,DBachievements,DBlevel,DBrole,DBidle,DBguildSetting,DBgift,DBserver);
                  console.log(`+ + + User: ${UserNameDS} | ${IndexUserID} + + +\n+ + + Guild: ${GuildNameDS} | ${IndexGuildID} + + +`)
            }
